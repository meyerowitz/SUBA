...
export default function WalletScreen() {
  ...
  const ejecutarTransferenciaReal = async () => {
    ...
    try {
      ...
      if (errorHistorial)
        console.log(
          "Aviso: No se pudo registrar en el historial, pero el dinero se movió.",
        );
  ...
  const ejecutarTransferenciaSimulada = () => {
    ...
    setTimeout(() => {
      ...
      Alert.alert(
        "¡Transferencia Exitosa!",
        `Has enviado Bs. ${monto} a ${nombreDestinatario}`,
      );
    }, 1500);
  };
    ...
    try {
      ...
      if (data) {
        const formateados = data.map((item) => ({
          ...
          icon:
            item.estado === "completado" ? "checkmark-circle" : "time-outline",
        }));
  ...
  const registrarYValidar2 = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Recarga Exitosa",
        `Tu saldo ha sido actualizado. Nuevo saldo: Bs. ${nuevoSaldoCalculado.toFixed(2)}`,
      );
  ...
  const renderItem = ({ item }) => (
    <View style={styles.transactionItem}>
      ...
      <Text style={[styles.itemAmount, { color: item.colorMonto }]}>
        {item.monto}
      </Text>
    </View>
  );
        ...
        ListHeaderComponent={
          <>
            ...
            <View style={styles.actionPanel}>
              ...
              <TouchableOpacity
                onPress={() => setModalTransferVisible(true)}
                style={styles.actionButton}
              >
                <View
                  style={[
      ...
      <Modal animationType="slide" transparent visible={modalTransferVisible}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <Text style={styles.modalSubtitle}>
              Ingresa el ID del usuario (Prueba con 12345 o 67890)
            </Text>
