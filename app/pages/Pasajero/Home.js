...
export default function Home({ navigation }) {
  ...
  const API_BASE_URL = "https://subapp-api.onrender.com/api";
  ...
  useEffect(() => {
    async function initNfc() {
      try {
        // 1. Comprobar si está soportado
        const supported = await NfcManager.isSupported();
        if (!supported) {
          console.log("NFC no soportado en este dispositivo");
          return;
        }

        // 2. Darle un respiro al bridge nativo antes de iniciar
        ...
        // Este catch atrapará el TypeError si sigue fallando
        console.warn("Error inicializando NFC:", ex);
      }
    }
  ...
  async function vincularTarjeta() {
    setVinculando(true);
    try {
      // A. Verificar soporte de hardware
      const supported = await NfcManager.isSupported();
      if (!supported) {
        Alert.alert(
          "Hardware no compatible",
          "Tu dispositivo no cuenta con hardware NFC.",
        );
        setVinculando(false);
        return;
      }

      // B. Verificar si está encendido
      const enabled = await NfcManager.isEnabled();
      if (!enabled) {
        Alert.alert(
          "NFC Desactivado",
          "Por favor, activa el NFC en la configuración de tu teléfono.",
        );
        setVinculando(false);
        return;
      }

      // C. RECUPERAR TOKEN Y USUARIO DE LA SESIÓN (Corregido)
      const sesionString = await AsyncStorage.getItem("@Sesion_usuario");
      if (!sesionString) {
        Alert.alert(
          "Error",
          "Sesión no encontrada. Por favor inicie sesión nuevamente.",
        );
        setVinculando(false);
        return;
      }
      ...
      const token = sesion.token; // 🔥 AQUÍ ESTÁ EL TOKEN
      const userId = sesion.id; // O sesion.user.id, depende de tu JSON de login
      ...
      if (tag) {
        console.log("UID Tarjeta:", tag.id);

        // E. Enviar a la API con Token en el Header
        const response = await axios.post(
          `${API_BASE_URL}/nfc/vincular`,
          {
            userId: userId, // Aunque el token ya dice quién es, a veces el backend lo pide
            cardUid: tag.id,
          },
          {
            headers: {
        ...
        if (response.data.success) {
          ...
          Alert.alert("Error", response.data.message || "Error en vinculación");
        }
      }
    } catch (ex) {
      console.warn(ex);
      Alert.alert("Error", "Error al leer la tarjeta o de conexión");
    } finally {
      NfcManager.cancelTechnologyRequest().catch(() => {});
      setVinculando(false);
    }
        ...
        >
          ...
          >
            ...
            >
              ...
              <TouchableOpacity
                onPress={() => router.push("/pages/Pasajero/Profile")}
              >
                {userImage ? (
                  <Image
                  ...
                  <View style={{ flexDirection: "row", alignItems: "center" }}>
                    ...
                    >
                      <Picker.Item
                        ...
                        color="#999"
                      />
                      {(selectedStop ? filteredRoutes : activeRoutes).map(
                        (dest) => (
                          <Picker.Item
                            key={dest.name}
                            label={dest.name}
          ...
          {/* --- NUEVO BOTÓN DE VINCULAR NFC --- */}
          <TouchableOpacity
            style={{
              backgroundColor: "#003366", // Azul oscuro para contrastar con el naranja
              marginHorizontal: 20,
