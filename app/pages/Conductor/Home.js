...
export default function Home() {
  ...
  useEffect(() => {
    ...
    mqttClient.on("error", (err) => {
      ...
      setErrorMsg("Error de conexión MQTT");
    });
  ...
  const sendLocation = useCallback(
    async (locationToSend, currentBusId) => {
      ...
      if (client && client.connected) {
        const topic = `subapp/driver`;
        client.publish(
          topic,
          JSON.stringify(payload),
          { qos: 0, retain: false },
          (error) => {
            if (error) console.error("Error publicando MQTT:", error);
            else console.log(`MQTT Publicado en ${topic}`);
          },
        );
      }
    },
  ...
  const moveBus = async () => {
    if (!myLocation) {
      setErrorMsg(
        "No se ha cargado la ubicación inicial para poder mover el bus.",
      );
      return;
    }
  ...
  return (
    <View style={styles.container}>
      ...
      <Text>
        Mi ID de Bus:{" "}
        <Text style={{ fontWeight: "bold", color: "#007AFF" }}>{busId}</Text>
      </Text>
      <Text>Ubicación Actual (Simulada):</Text>
      {myLocation ? (
        <Text style={{ marginBottom: 10, color: "#333" }}>
          Lat: {myLocation.latitude.toFixed(6)}, Lon:{" "}
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          ...
          >
            ...
            {item.last_location && item.last_location.coordinates && (
              <Text>
                📍 Lat: {item.last_location.coordinates[1].toFixed(6)}, Lon:{" "}
                {item.last_location.coordinates[0].toFixed(6)}
              </Text>
            )}
            <Text>
              ⚡ Velocidad:{" "}
              {item.last_speed !== undefined
                ? item.last_speed.toFixed(1)
                : "N/A"}{" "}
              km/h
            </Text>
            <Text>
        ...
        ListEmptyComponent={() => (
          <Text style={styles.emptyText}>Esperando datos de buses...</Text>
        )}
      />
      <View
        style={{ flexDirection: "row", marginHorizontal: 90, marginBottom: 20 }}
      >
        <Button
